...
// KauriNodeClient is the single node client interface for the Kauri service.
type KauriNodeClient interface {
	SendContribution(ctx context.Context, in *Contribution, opts ...gorums.CallOption)
}

// enforce interface compliance
var _ KauriNodeClient = (*Node)(nil)

// There are no quorum calls.
type QuorumSpec interface{}

// Kauri is the server-side API for the Kauri Service
type KauriServer interface {
	SendContribution(ctx gorums.ServerCtx, request *Contribution)
}

func RegisterKauriServer(srv *gorums.Server, impl KauriServer) {
	srv.RegisterHandler("kauripb.Kauri.SendContribution", func(ctx gorums.ServerCtx, in *gorums.Message) (*gorums.Message, error) {
		req := gorums.AsProto[*Contribution](in)
